...
# Feature flags default to 0, will try to auto-detect from header file
MLOG_ENABLE_DESCRIPTION = 0
MLOG_ENABLE_MODEL_INFO = 0
MLOG_ENABLE_PARAM = 0
...
def parse_header(data: bytes) -> Tuple[int, int, int, int, int, int, Dict[int, BusDef], int]:
    # Returns: (version, timestamp, max_name_len, max_desc_len, max_model_info_len, num_bus, bus_map, offset_after_header)
    offset = 0
    ...
    # Parameter section disabled in this project; if enabled, would parse here.
    return (
        version,
        timestamp,
        max_name_len,
...
def main() -> int:
    ...
    try:
        version, ts, max_name_len, max_desc_len, max_model_info_len, num_bus, bus_map, off = parse_header(data)
    except EOFError:
        print("文件过短，无法解析头部。")
        return 2
