...
static int mlog_test_echo(void *parameter) {
    ...
    rt_kprintf("Mlog Test Echo - acc_raw: %s, %s, %s, gyro_raw: %s, %s, %s, timestamp: %lu\n", 
               ax, ay, az, gx, gy, gz, test_data.timestamp);
...
static void mlog_test_rtos_init(void) {
    ...
    mlog_test_timer = rt_timer_create("mlog_test_tmr", mlog_test_timer_cb, RT_NULL, 
                                      period_ticks, RT_TIMER_FLAG_PERIODIC | RT_TIMER_FLAG_SOFT_TIMER);
...
static void mlog_test_thread_entry(void *parameter) {
    ...
    while (1) {
        ...
        rt_event_recv(&mlog_test_event, MLOG_TEST_EVENT_FLAG_TRIGGER, 
...
rt_err_t mlog_test_start(void) {
    ...
    if (!mlog_test_thread_created) {
        ...
        rt_thread_init(&task_tid_mlog_test, "mlogTest", mlog_test_thread_entry, RT_NULL, 
                       task_stack_mlog_test, MLOG_TEST_THREAD_STACK_SIZE, 
                       MLOG_TEST_THREAD_PRIORITY, MLOG_TEST_THREAD_TIMESLICE);
