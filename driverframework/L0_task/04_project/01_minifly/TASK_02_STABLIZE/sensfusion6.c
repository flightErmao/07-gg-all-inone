...
// 仅在加速度模长接近1g时信任加速度（单位g），并对积分环节做泄放和限幅
#define ACC_NORM_MIN 0.90f
#define ACC_NORM_MAX 1.10f
#define INTEGRATOR_LEAK 0.999f
#define INTEGRATOR_LIMIT 0.10f
...
static void calBaseAcc(float* acc) /*计算静态加速度*/
{
  ...
  for (uint8_t i = 0; i < 3; i++) sumAcc[i] += acc[i];
  ...
  if (++cnt >= ACCZ_SAMPLE) {
    ...
    if (maxError < 0.015f) {
      for (uint8_t i = 0; i < 3; i++) baseAcc[i] = sumAcc[i] / ACCZ_SAMPLE;

      isGravityCalibrated = true;
    }

    for (uint8_t i = 0; i < 3; i++) sumAcc[i] = 0.f;
  }
}
...
void imuUpdate(Axis3f acc, Axis3f gyro, state_t* state, float dt) /*数据融合 互补滤波*/
{
  ...
  // 先根据未归一化的加速度模长，判断该周期是否信任加速度测量
  float accNorm = sqrtf(tempacc.x * tempacc.x + tempacc.y * tempacc.y + tempacc.z * tempacc.z);
  bool accValid = (accNorm > ACC_NORM_MIN) && (accNorm < ACC_NORM_MAX);
  ...
  if (accValid) {
    ...
    /*误差累计，与积分常数相乘，并做泄放和限幅，避免积分风up*/
    exInt = exInt * INTEGRATOR_LEAK + Ki * ex * dt;
    eyInt = eyInt * INTEGRATOR_LEAK + Ki * ey * dt;
    ezInt = ezInt * INTEGRATOR_LEAK + Ki * ez * dt;

    if (exInt > INTEGRATOR_LIMIT)
      exInt = INTEGRATOR_LIMIT;
    else if (exInt < -INTEGRATOR_LIMIT)
      exInt = -INTEGRATOR_LIMIT;
    if (eyInt > INTEGRATOR_LIMIT)
      eyInt = INTEGRATOR_LIMIT;
    else if (eyInt < -INTEGRATOR_LIMIT)
      eyInt = -INTEGRATOR_LIMIT;
    if (ezInt > INTEGRATOR_LIMIT)
      ezInt = INTEGRATOR_LIMIT;
    else if (ezInt < -INTEGRATOR_LIMIT)
      ezInt = -INTEGRATOR_LIMIT;
    ...
    // 当线加速度较大或振动强时，不使用加速度做修正，仅缓慢泄放积分，防止长期漂移
    exInt *= INTEGRATOR_LEAK;
    eyInt *= INTEGRATOR_LEAK;
    ezInt *= INTEGRATOR_LEAK;
  }
...
bool getIsCalibrated(void) { return isGravityCalibrated; }
