...
void VI530x_main(void)
{
...
#else	
	ret |= VI530x_Chip_SWReset();			//Xshut引脚不启用，软复位，Xshut引脚需要上拉
#endif
	...
	/*******请根据应用需求评估测试使用**************/
	ret |= VI530x_Set_Integralcounts_Frame(30,131072);	//帧率，积分次数，默认(30,131072)
	...
	/*******没有标定前用于测试的缺省标定值**************/
		...
		//RefToF标定没做前请在关闭温补功能。
		VI530x_Cali_Data.VI530x_Calibration_Reftof = 0;
	#ifdef VI530x_GRADIENTK_CALIBRATION
		VI530x_Cali_Data.VI530x_Calibration_GradientK = 1;
	#endif
#endif
	/* End user code.  */
	...
	ret |= VI530x_Set_Californiation_Data(VI530x_Cali_Data);
...
#ifdef VI530x_GRADIENTK_CALIBRATION
	//必须先完成Offset标定，Gradient标定距离和Offset标定需要不一样。
	//Gradient_k标定,可以根据应用需求标定关键距离和目标。
	//在固定距离做Gradient标定,如20cm，则把参数VI530x_SLOPEK_DISTANCE改成200（mm）
	ret |= VI530x_GradientK_Calibration(VI530x_OFFSET_DISTANCE, VI530x_GRADIENT_DISTANCE);
	if(ret == 0)
	{
		printf("VI530x_Calibration_GradientK = %f\r\n",VI530x_Cali_Data.VI530x_Calibration_GradientK);
		printf("VI530x_Calibration_Offset = %f\r\n",VI530x_Cali_Data.VI530x_Calibration_Offset);
	}
	else
	{
		printf("Gradient Calibration Fail!\r\n");
	}
#endif
	/*******************保存标定值在主控NVM*************************/
	...
	VI530x_Cali_Data.VI530x_Calibration_SlopeK			//默认1，正常不需要使用
	********************************************/
	/* Start user code for adding. */
