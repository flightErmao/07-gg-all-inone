...
/* Global TOF data for external access (used by acSendLog.c) */
static VI530x_MEASURE_TypeDef g_tof_data = {0};
static rt_bool_t g_tof_data_valid = RT_FALSE;
...
#ifdef WORK_TASK_TOF_XL530X_USE_INTERRUPT
  /* 初始化事件（仅在中断模式下需要） */
  if (!g_tof_event_inited) {
  ...
  // 3、选择中断方式：0x88----GPIO硬件中断，0x00----寄存器查询
    ...
    while (1) {
      ...
      if (g_tof_event_inited) {
        ...
        if (!ret) {
          /* 更新全局TOF数据（供acSendLog.c使用） */
          g_tof_data = result;
          g_tof_data_valid = RT_TRUE;

#ifndef TASK_TOOL_01_ANOTC_TELEM_EN
          /* 如果未启用ANOTC_TELEM，则通过rt_kprintf输出数据 */
...
{
#ifdef WORK_TASK_TOF_XL530X_AUTO_START
  rt_thread_t thread =
      rt_thread_create("tof_xl530x", tof_thread_entry, RT_NULL, THREAD_STACK_SIZE, THREAD_PRIORITY, THREAD_TIMESLICE);
  if (thread != RT_NULL) {
    tof_task_thread = thread;
    rt_thread_startup(thread);
    rt_kprintf("[TOF_XL530X] thread started automatically, i2c:%s, addr:0x%02X\n", WORK_TASK_TOF_XL530X_I2C_NAME,
               VI530x_I2C_ADDR);
  } else {
...
#else
  rt_kprintf("[TOF_XL530X] Task initialized (manual start via 'tof start')\n");
  rt_kprintf("[TOF_XL530X] i2c:%s, addr:0x%02X\n", WORK_TASK_TOF_XL530X_I2C_NAME, VI530x_I2C_ADDR);
#endif
