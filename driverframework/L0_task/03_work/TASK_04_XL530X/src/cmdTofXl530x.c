...
static void cmd_tof_start(int argc, char **argv)
{
    ...
    rt_thread_t thread = rt_thread_find("tof_xl530x");
    if (thread != RT_NULL) {
    ...
    thread = rt_thread_create("tof_xl530x", tof_thread_entry, RT_NULL,
...
#ifdef WORK_TASK_TOF_XL530X_CMD_REFTOF_CALIB
/* RefToF calibration command */
static void cmd_tof_reftof_calib(int argc, char **argv)
{
    rt_kprintf("[%s] RefToF calibration is not available in XL530X version\n", TAG);
    rt_kprintf("[%s] This feature is only available in XL5300 version\n", TAG);
    rt_kprintf("[%s] Current Reftof value: %4d\n", TAG, VI530x_Cali_Data.VI530x_Cali_Reftof);
}
MSH_CMD_EXPORT_ALIAS(cmd_tof_reftof_calib, tof_reftof, RefToF calibration (not available in XL530X));
#endif

#ifdef WORK_TASK_TOF_XL530X_CMD_XTALK_CALIB
...
#ifdef WORK_TASK_TOF_XL530X_CMD_OFFSET_CALIB
...
#ifdef WORK_TASK_TOF_XL530X_CMD_GRADIENT_CALIB
...
static void cmd_tof_show_calib(int argc, char **argv)
{
    ...
    rt_kprintf("  Reftof:        %d\n", VI530x_Cali_Data.VI530x_Cali_Reftof);
    rt_kprintf("  Offset:        %f\n", VI530x_Cali_Data.VI530x_Cali_Offset);
#ifdef VI530x_GRADIENTK_CALIBRATION
    rt_kprintf("  GradientK:     %f\n", VI530x_Cali_Data.VI530x_Cali_GradientK);
#endif
}
...
MSH_CMD_EXPORT_ALIAS(cmd_tof, tof, TOF XL530X control and calibration);
