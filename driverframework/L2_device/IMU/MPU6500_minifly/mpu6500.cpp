...
mpu6500& mpu6500::instance() {
  static mpu6500 s_instance;
  return s_instance;
}

// 设置延时函数指针
void mpu6500::setDelayMs(delay_ms_func_t func) { delay_ms_ = func; }

// 内部延时函数
void mpu6500::delayMs(unsigned int ms) {
...
int mpu6500::read_data(int pos, void* data, int size) {
  if (data == nullptr) {
    return 0;
  }
...
int8_t mpu6500::setI2cBusWrite(int8_t (*func)(uint8_t, uint8_t, uint8_t*, uint8_t)) {
  if (func == nullptr) {
    return -1;
  } else {
    i2c_bus_write_ = func;
...
int8_t mpu6500::setI2cBusRead(int8_t (*func)(uint8_t, uint8_t, uint8_t*, uint8_t)) {
  if (func == nullptr) {
    return -1;
  } else {
    i2c_bus_read_ = func;
...
int8_t mpu6500::i2cBusRead(uint8_t devAddress, uint8_t memAddress, uint8_t len, uint8_t* data) {
  if (i2c_bus_read_) {
...
int8_t mpu6500::i2cBusWrite(uint8_t devAddress, uint8_t memAddress, uint8_t len, uint8_t* data) {
  if (i2c_bus_write_) {
...
int8_t mpu6500::i2cBusReadByte(uint8_t devAddress, uint8_t memAddress, uint8_t* data) {
  return i2cBusRead(devAddress, memAddress, 1, data);
}

int8_t mpu6500::i2cBusWriteByte(uint8_t devAddress, uint8_t memAddress, uint8_t data) {
  return i2cBusWrite(devAddress, memAddress, 1, &data);
}

void mpu6500::setI2cAddr(uint8_t addr) { dev_addr_ = addr; }

// 新增位操作函数
int8_t mpu6500::i2cdevReadBit(uint8_t devAddr, uint8_t regAddr, uint8_t bitNum, uint8_t* data) {
  uint8_t b;
...
int8_t mpu6500::i2cdevReadBits(uint8_t devAddr, uint8_t regAddr, uint8_t bitStart, uint8_t length, uint8_t* data) {
  uint8_t b;
  uint8_t mask;
  int8_t status = i2cBusReadByte(devAddr, regAddr, &b);
  if (status == 0) {
...
int8_t mpu6500::i2cdevWriteBit(uint8_t devAddr, uint8_t regAddr, uint8_t bitNum, uint8_t data) {
  uint8_t b;
  int8_t status = i2cBusReadByte(devAddr, regAddr, &b);
  if (status == 0) {
...
int8_t mpu6500::i2cdevWriteBits(uint8_t devAddr, uint8_t regAddr, uint8_t bitStart, uint8_t length, uint8_t data) {
  uint8_t b;
  uint8_t mask;
  int8_t status = i2cBusReadByte(devAddr, regAddr, &b);
  if (status == 0) {
...
// 公开的数据获取函数
uint8_t mpu6500::getAccelerometer(int16_t* ax, int16_t* ay, int16_t* az) {
  uint8_t buf[6];
...
uint8_t mpu6500::getGyroscope(int16_t* gx, int16_t* gy, int16_t* gz) {
  uint8_t buf[6];
...
uint8_t mpu6500::getTemperature(float* temp) {
  uint8_t buf[2];
...
void mpu6500::setIntEnabled(bool enabled) { i2cBusWriteByte(dev_addr_, MPU6500_RA_INT_ENABLE, enabled ? 0x01 : 0x00); }
...
void mpu6500::setAccelDLPF(uint8_t bandwidth) {
...
uint8_t mpu6500::setRate(uint16_t rate) {
  uint8_t data;
  if (rate == 0) {
...
// 兼容C接口（基于单例）
extern "C" int drv_mpu6500_init() { return mpu6500::instance().init(); }

extern "C" int drv_mpu6500_set_i2c_addr(uint8_t addr) {
  mpu6500::instance().setI2cAddr(addr);
  return MPU_EOK;
}

extern "C" int drv_mpu6500_set_delay(void (*delay_ms_cb)(unsigned int)) {
  mpu6500::instance().setDelayMs(delay_ms_cb);
  return MPU_EOK;
}

extern "C" int drv_mpu6500_set_i2c_funcs(int8_t (*write_func)(uint8_t, uint8_t, uint8_t*, uint8_t),
                                         int8_t (*read_func)(uint8_t, uint8_t, uint8_t*, uint8_t)) {
  int8_t w = mpu6500::instance().setI2cBusWrite(write_func);
  int8_t r = mpu6500::instance().setI2cBusRead(read_func);
  return (w == 0 && r == 0) ? MPU_EOK : MPU_ERROR;
}

extern "C" int drv_mpu6500_read(int pos, void* data, int size) {
  return mpu6500::instance().read_data(pos, data, size);
}

extern "C" int drv_mpu6500_get_accel(int16_t* ax, int16_t* ay, int16_t* az) {
  uint8_t res = mpu6500::instance().getAccelerometer(ax, ay, az);
  return (res == 0) ? MPU_EOK : MPU_ERROR;
}

extern "C" int drv_mpu6500_get_gyro(int16_t* gx, int16_t* gy, int16_t* gz) {
  uint8_t res = mpu6500::instance().getGyroscope(gx, gy, gz);
  return (res == 0) ? MPU_EOK : MPU_ERROR;
}

extern "C" int drv_mpu6500_get_temp(float* temp) {
  uint8_t res = mpu6500::instance().getTemperature(temp);
  return (res == 0) ? MPU_EOK : MPU_ERROR;
}
