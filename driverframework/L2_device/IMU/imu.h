...
struct imu_configure {
    // TODO: add gain(needed by bmi270 if we don't wanna trigger CRT everytime we power up the sensor)
    rt_uint32_t gyro_odr_hz;
    rt_uint16_t gyro_dlpf_mode;     /* can be one of some modes (e.g. Normal/OSR2/OSR4 mode ) or just frequency number (e.g. 100Hz/343Hz) */
    // TODO: figure out: why odr is 32bit and why dlpf is 16bit?
    rt_uint32_t accel_odr_hz;
    rt_uint16_t accel_dlpf_mode;    /* can be one of some modes (e.g. Normal/OSR2/OSR4 mode ) or just frequency number (e.g. 100Hz/343Hz) */
...
struct imu_ops {
    ...
    */
    rt_err_t (*imu_config)(imu_dev_t dev, const struct gyro_configure* cfg);
    // /**
