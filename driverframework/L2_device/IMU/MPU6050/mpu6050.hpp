...
#define MPU6500_DMP_CONFIG_SIZE 192
  ...
  class MPU6050 {
   public:
    explicit MPU6050(const char* i2c_device_name);
    ~MPU6050();

    rt_err_t init();
    rt_err_t config(const imu_configure* cfg);
    rt_size_t read_data(rt_off_t pos, void* data, rt_size_t size);

    void setI2cBusWrite(int8_t (*func)(uint8_t, uint8_t, uint8_t*, uint8_t));
    void setI2cBusRead(int8_t (*func)(uint8_t, uint8_t, uint8_t*, uint8_t));
    void setI2cAddr(uint8_t addr);

   private:
    rt_device_t imu_i2c_dev_ = nullptr;
    uint8_t dev_addr_ = {};
    uint8_t buffer_[14] = {0};
    bool is_init_ = false;
    imu_configure config_{};
