...
static rt_ssize_t hal_baro_read(struct rt_device* dev, rt_off_t pos, void* buffer, rt_size_t size) {
  rt_size_t rb = 0;
  baro_dev_t baro;
  ...
  if (pos == BARO_RD_REPORT && baro->ops->baro_read) {
    rb = baro->ops->baro_read(baro, (baro_report_t*)buffer);
  }

  return rb;
}

static rt_err_t hal_baro_control(struct rt_device* dev, int cmd, void* args) {
  rt_err_t ret = RT_EOK;
  baro_dev_t baro;
