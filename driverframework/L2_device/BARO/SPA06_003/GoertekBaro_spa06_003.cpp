...
GoertekBaro::GoertekBaro(int id) : id_(id)
{
    dev_mode_ = GoertekBaro_MODE_IDLE;
    pressure_mr_ = PM_RATE_32_HZ;
    pressure_osr_ = OSR_16_TIMES;
    temperature_mr_ = TMP_RATE_16_HZ;
    temperature_osr_ = OSR_2_TIMES;
}
...
bool GoertekBaro::Read(BaroData &data)
{
    ...
    if (baro_model_ == SPL06_001)
    {
        data.pressure_pa =
            calib_coeffs_.c00 +
            pressure_scaled *
                (calib_coeffs_.c10 + pressure_scaled * (calib_coeffs_.c20 + pressure_scaled * calib_coeffs_.c30)) +
    ...
    else if (baro_model_ == SPA06_003)
    {
        data.pressure_pa =
            ...
            pressure_scaled *
                (calib_coeffs_.c10 +
                 pressure_scaled * (calib_coeffs_.c20 +
                                    pressure_scaled * (calib_coeffs_.c30 + pressure_scaled * calib_coeffs_.c40))) +
            temperature_scaled * calib_coeffs_.c01 +
...
void GoertekBaro::funRegisterTimer(timeDelayMs delay_ms_func)
{
    delay_msec = delay_ms_func;
}

GoertekBaro::GoertekBaro() : id_(0)
{
    dev_mode_ = GoertekBaro_MODE_IDLE;
    pressure_mr_ = PM_RATE_32_HZ;
    pressure_osr_ = OSR_16_TIMES;
